...
class IndexSnippet extends SourceSnippet {
  ...
  findAppOrRouter(targetSource) {
    ...
    traverse(targetSource.ast, {
      JSXOpeningElement: (path) => {
        if (
          path.get('name').get('name').node === 'Router' ||
          path.get('name').get('name').node === 'App'
        ) {
          appElement = path;
        }
      },
    ...
    if (!appElement) {
      throw new Error(
        `App class not found. Can't parse dashboard app. Please delete the dashboard-app directory and try to create it again.`
      );
    }
