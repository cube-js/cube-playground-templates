...
class CredentialsSnippet extends SourceSnippet {
  ...
  replaceTokens(targetSource) {
    const apiUrl = targetSource.definitions.find(
      (d) => d.get('id').node.name === 'API_URL'
    );
    apiUrl.get('init').replaceWith(t.stringLiteral(this.apiUrl));

    const cubejsToken = targetSource.definitions.find(
      (d) => d.get('id').node.name === 'CUBEJS_TOKEN'
    );
