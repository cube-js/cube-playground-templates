...
class ChartRendererSnippet extends SourceSnippet {
  handleExistingMerge(existingDefinition, newDefinition) {
    if (existingDefinition.get('id').node.name === 'TypeToChartComponent') {
      const existingPropertyNames = existingDefinition
        .get('init')
        .get('properties')
        .map((p) => p.node.key.name);
      existingDefinition.get('init').pushContainer(
        'properties',
        newDefinition.node.init.properties.filter(
          (p) => existingPropertyNames.indexOf(p.key.name) === -1
        )
      );
    } else {
  ...
  insertAnchor(targetSource) {
    ...
    if (!anchor) {
      throw new Error(
        `renderChart class not found. Can't parse dashboard app.  Please delete dashboard-app directory and try to create it again.`
      );
    }
    return anchor;
  }
